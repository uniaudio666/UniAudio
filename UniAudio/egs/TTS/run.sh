...
train_set="train"
valid_set="train_holdout"
test_sets=""

# training config
seed=999
debug=false
batch_scale=6000 # the total number of tokens in one batch
    ...
    for part in $valid_set $train_set; do
      for n in `seq 0 $[$ngpu-1]`; do
        ...
        python3 data_scripts/create_data_json.py \
         --task plain_tts \
         --out-json   $PWD/data/${part}/${ngpu}splits/data.${n}.json \
...
train_data_jsons="data/${train_set}/${ngpu}splits/data_plain_exmple.ALL.json"
valid_data_jsons="data/${valid_set}/${ngpu}splits/data_plain_exmple.ALL.json"
...
inference_tag="plain_tts_inference"
